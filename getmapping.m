...
if strcmp(mappingtype,'Iu2') %Improved Uniform 2
    newMax = samples*(samples-1) + 3;
    for i = 0:2^samples-1
        j = bitset(bitshift(i,1,samples),1,bitget(i,samples)); %rotate left
        numt = sum(bitget(bitxor(i,j),1:samples));  %number of 1->0 and
        ...
        if numt <= 2
            ...
            % by leezonpen 2016.3.31
            % for improved lbp
            table(i+1) = newMax  + numt/2 -2;
        end
    end
end
...
if strcmp(mappingtype,'Iriu2') %Improved Uniform & Rotation invariant
    newMax = samples + 2;
    for i = 0:2^samples - 1
        j = bitset(bitshift(i,1,samples),1,bitget(i,samples)); %rotate left
        numt = sum(bitget(bitxor(i,j),1:samples));
